...
checkpoint="${CHECKPOINT:?Set CHECKPOINT to the validation-selected v1.7 checkpoint}"

mkdir -p "${experiment_dir}/evaluation"
CUDA_VISIBLE_DEVICES="${device}" python test_bridgeadaptclip_v11.py \
    --model_name BridgeAdaptCLIP-v1.7 \
    --checkpoint_state_key bridgeadaptclip_v17 \
