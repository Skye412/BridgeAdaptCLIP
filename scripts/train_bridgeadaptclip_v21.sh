...
experiment_dir="${EXPERIMENT_DIR:?Set EXPERIMENT_DIR to an initialized v2.1 directory}"
row0_checkpoint="${ROW0_CHECKPOINT:?Set ROW0_CHECKPOINT to Protocol-v2 Row0 Epoch 14}"
fine_checkpoint="${FINE_CHECKPOINT:?Set FINE_CHECKPOINT to validation-selected v2.1-Fine Epoch 9}"
epochs="${EPOCHS:-15}"
mkdir -p "${experiment_dir}/checkpoints"
CUDA_VISIBLE_DEVICES="${device}" python train_bridgeadaptclip_v20.py \
  --model_name BridgeAdaptCLIP-v2.1 \
  --checkpoint_state_key bridgeadaptclip_v21 \
  ...
  --features_list 6 12 18 24 --model_input_size 518 --structural_input_size 1024 \
  --fusion_channels 128 --structural_channels 128 --broad_channels 128 --strip_kernel 5 \
